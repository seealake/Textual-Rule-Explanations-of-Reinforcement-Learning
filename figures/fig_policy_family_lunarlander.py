#!/usr/bin/env python
"""PPO vs DQN on LunarLander-v3.

Grouped bar chart comparing CBS/DT/RV stability metrics
between PPO and DQN on LunarLander-v3 (mirrors the CartPole figure).
"""
import sys, os
...
def main():
    ...
    # Load PPO results (LunarLander)
    ppo_path = RESULTS_DIR / "cross_algo_comparison" / "ppo_lunarlander_stress.json"
    ppo_data = load_json(str(ppo_path))

    # Load DQN results (LunarLander)
    dqn_path = RESULTS_DIR / "lunarlander_v3" / "stress_test_results.json"
    dqn_data = load_json(str(dqn_path))

    # Extract CBS stability
    ppo_cbs = ppo_data.get("CBS", {}).get("stability", {})
    dqn_cbs_raw = dqn_data.get("cbs", {})
    dqn_cbs = dqn_cbs_raw.get("stability", dqn_cbs_raw)
    ...
    # Panel A: CBS PPO vs DQN on LunarLander
    ax = axes[0]
    ...
    # Panel B: PPO methods comparison (CBS vs DT vs RV) on LunarLander
    ax = axes[1]
    methods = ["CBS", "DT", "RV"]
    ...
    ax.set_ylabel("BRA")
    ax.set_ylim(0, 1.05)
    ax.set_title("(b) PPO Methods (LunarLander)", fontsize=9)

    for i, v in enumerate(bra_vals):
        ax.text(i, v + 0.02, f"{v:.2f}", ha="center", va="bottom", fontsize=9)

    fig.tight_layout()
    savefig(fig, "fig_policy_family_lunarlander")
    print("  Generated fig_policy_family_lunarlander")
